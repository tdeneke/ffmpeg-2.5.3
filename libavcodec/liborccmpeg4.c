...
#if defined(_MSC_VER)
#define ORCCMPEG4_API_IMPORTS 1
#endif
...
static orccmpeg4_param_t *cmd;
static AVPacket *tmp_avpkt; 

////////////////////////////////////////////////////////////////////////////////
// Instance
extern actor_t MPEG4Source;

////////////////////////////////////////////////////////////////////////////////
// Output FIFOs
extern fifo_u8_t *MPEG4Source_O;
...
#define SIZE_O SIZE
#define tokens_O MPEG4Source_O->contents

////////////////////////////////////////////////////////////////////////////////
// Successors
extern actor_t MPEG4Decoder_serialize;

////////////////////////////////////////////////////////////////////////////////
// Token functions

static void write_O() {
	index_O = MPEG4Source_O->write_ind;
	numFree_O = index_O + fifo_u8_get_room(MPEG4Source_O, NUM_READERS_O);
}

static void write_end_O() {
	MPEG4Source_O->write_ind = index_O;
}
...
static void MPEG4Source_initialize(schedinfo_t *si) {
	int i = 0;
	write_O();
	source_init();
finished:
...
////////////////////////////////////////////////////////////////////////////////
// Action scheduler
static void MPEG4Source_scheduler(schedinfo_t *si) {
	int i = 0;
	si->ports = 0;
	...
	while (1) {
		if (isSchedulable_s()) {
			int stop = 0;
			if (1 > SIZE_O - index_O + MPEG4Source_O->read_inds[0]) {
				stop = 1;
			}
			if (stop != 0) {
				si->num_firings = i;
...
////////////////////////////////////////////////////////////////////////////////
// Instance
extern actor_t MPEG4Display;

////////////////////////////////////////////////////////////////////////////////
// Input FIFOs
extern fifo_u8_t *MPEG4Display_B;
extern fifo_i16_t *MPEG4Display_WIDTH;
extern fifo_i16_t *MPEG4Display_HEIGHT;
...
#define SIZE_B SIZE
#define tokens_B MPEG4Display_B->contents

extern connection_t connection_MPEG4Display_B;
#define rate_B connection_MPEG4Display_B.rate

static unsigned int index_WIDTH;
static unsigned int numTokens_WIDTH;
#define SIZE_WIDTH SIZE
#define tokens_WIDTH MPEG4Display_WIDTH->contents

extern connection_t connection_MPEG4Display_WIDTH;
#define rate_WIDTH connection_MPEG4Display_WIDTH.rate

static unsigned int index_HEIGHT;
static unsigned int numTokens_HEIGHT;
#define SIZE_HEIGHT SIZE
#define tokens_HEIGHT MPEG4Display_HEIGHT->contents

extern connection_t connection_MPEG4Display_HEIGHT;
#define rate_HEIGHT connection_MPEG4Display_HEIGHT.rate

////////////////////////////////////////////////////////////////////////////////
// Predecessors
extern actor_t MPEG4Decoder_Merger420;
extern actor_t MPEG4Decoder_parser_parseheaders;
extern actor_t MPEG4Decoder_parser_parseheaders;
...
// Token functions
static void read_B() {
	index_B = MPEG4Display_B->read_inds[0];
	numTokens_B = index_B + fifo_u8_get_num_tokens(MPEG4Display_B, 0);
}

static void read_end_B() {
	MPEG4Display_B->read_inds[0] = index_B;
}
static void read_WIDTH() {
	index_WIDTH = MPEG4Display_WIDTH->read_inds[11];
	numTokens_WIDTH = index_WIDTH + fifo_i16_get_num_tokens(MPEG4Display_WIDTH, 11);
}

static void read_end_WIDTH() {
	MPEG4Display_WIDTH->read_inds[11] = index_WIDTH;
}
static void read_HEIGHT() {
	index_HEIGHT = MPEG4Display_HEIGHT->read_inds[3];
	numTokens_HEIGHT = index_HEIGHT + fifo_i16_get_num_tokens(MPEG4Display_HEIGHT, 3);
}

static void read_end_HEIGHT() {
	MPEG4Display_HEIGHT->read_inds[3] = index_HEIGHT;
}
...
static void MPEG4Display_initialize(schedinfo_t *si) {
	int i = 0;
...
////////////////////////////////////////////////////////////////////////////////
// Action scheduler
static void MPEG4Display_outside_FSM_scheduler(schedinfo_t *si) {
	int i = 0;
	while (1) {
...
static void MPEG4Display_scheduler(schedinfo_t *si) {
	int i = 0;
	...
	default:
		printf("unknown state in MPEG4Display.c : %s\n", stateNames[_FSM_state]);
		exit(1);
	}

	// FSM transitions
l_GetChroma1Block:
	MPEG4Display_outside_FSM_scheduler(si);
	i += si->num_firings;
...
static av_cold int liborccmpeg4_decode_init(AVCodecContext *avctx)
{
    liborccmpeg4Context *q = avctx->priv_data;
    q->frames_decoded = 0;
    cmd = (orccmpeg4_param_t*) malloc(sizeof(orccmpeg4_param_t));
    ...
    orcc_thread_create(q->launch_thread, orccmpeg4_start_actors, *cmd, q->launch_thread_id);
    ...
    q->frames_decoded = 0;
    MPEG4Display_initialize(q->sink_sched_info);
    ...
    MPEG4Source_initialize(q->source_sched_info);

    return 0;
}

static int liborccmpeg4_decode_frame(AVCodecContext *avctx, void *data, int *got_frame, AVPacket *avpkt)
{
    liborccmpeg4Context *q = avctx->priv_data;
    ...
    to_send = avpkt->size;
    sent = 0;
    //printf("inside frame decoder orccmpeg4 tosend=%d sent=%d \n", tosend, sent);
    MPEG4Source_scheduler(q->source_sched_info);
    q->frames_decoded = nbFrameDecoded;
    MPEG4Display_scheduler(q->sink_sched_info);

    if(nbFrameDecoded > q->frames_decoded){
	
         //printf("inside frame decoder orccmpeg4 %d-%d \n", nbFrameDecoded, q->frames_decoded); 
         AVFrame* frame = data;
...
static av_cold int liborccmpeg4_decode_end(AVCodecContext *avctx)
{
    liborccmpeg4Context *q = avctx->priv_data;
...
AVCodec ff_liborccmpeg4_decoder = {
    .name                  = "liborccmpeg4",
    .long_name             = NULL_IF_CONFIG_SMALL("ORCC / MPEG4 / PART 2 SP"),
    .type                  = AVMEDIA_TYPE_VIDEO,
    .id                    = AV_CODEC_ID_ORCCMPEG4,
    .priv_data_size        = sizeof(liborccmpeg4Context),
    .init                  = liborccmpeg4_decode_init,
    .close                 = liborccmpeg4_decode_end,
    .decode                = liborccmpeg4_decode_frame,
